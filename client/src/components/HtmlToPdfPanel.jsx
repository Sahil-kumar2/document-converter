...
export default function HtmlToPdfPanel({ pdfFile, loading, setLoading, setResult, setResultBlob }) {
  const handleConvert = async () => {
    if (!pdfFile) {
      setResult({ success: false, error: "Please select an HTML file" });
      return;
    }
  ...
  return (
    <>
      <h3 className="font-semibold text-gray-900 mb-4">HTML to PDF</h3>
      ...
      {!pdfFile && (
        <div className="bg-blue-50 p-4 rounded text-center text-blue-800">
          <p>Upload an HTML file above to convert to PDF</p>
        </div>
      )}
    </>
  );
