...
export default function MergePdfPanel({
  ...
  useEffect(() => {
    ...
    setMergePdfFiles((prev) => addUniqueFiles(prev, file));
  }, [file]);
  ...
  const handleAddFiles = (e) => {
    ...
    if (pdfFiles.length !== files.length) {
      setResult({
        success: false,
        error: "Some files were not PDFs and were skipped",
      });
    }
  ...
  return (
    <>
      ...
      {mergePdfFiles.length > 0 && (
        <div className="mb-4 border border-gray-300 rounded p-3 max-h-48 overflow-y-auto">
          {mergePdfFiles.map((fileItem, index) => (
            <div
              key={`${fileItem.name}-${fileItem.size}-${fileItem.lastModified}`}
              className="flex items-center justify-between p-2 mb-1 bg-gray-100 rounded"
            >
      ...
      <p className="text-sm text-gray-600 mb-4">
        Files to merge: {mergePdfFiles.length}
      </p>
